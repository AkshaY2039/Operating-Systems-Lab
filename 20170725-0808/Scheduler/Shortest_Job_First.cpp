...
void Scheduler::Shortest_Job_First()
{
	...
	if(Number_of_Processes>0)
	{
		...
		cout<<"\nGantt Chart for Shortest_Job_First\n["<<System_Time<<"]";
		while(1)
		{
			Stop_Flag=1;
			Arrive_Flag=0;
			...
			for(int i=0;i<Number_of_Processes;i++)
				if(Arrival_Time[i]<=System_Time && IsCompleted[i]==0 && Service_Time[i]<Min_Service_Time)
					Min_Service_Time=Service_Time[i];
			
			if(Min_Service_Time!=INT_MAX)
			{	
				Min_Arrival_Time=INT_MAX;
				for(int i=0;i<Number_of_Processes;i++)
					if(Service_Time[i]==Min_Service_Time && Arrival_Time[i]<Min_Arrival_Time && IsCompleted[i]==0)
					{
